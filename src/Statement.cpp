...
auto Statement::BindBoolean(
    ...
    std::string_view name) -> CassError
{
    return cass_statement_bind_bool_by_name_n(m_cass_statement_ptr.get(), name.data(), name.length(), static_cast<cass_bool_t>(value));
}
...
auto Statement::BindText(
    ...
    std::string_view name) -> CassError
{
    return cass_statement_bind_string_by_name_n(m_cass_statement_ptr.get(), name.data(), name.length(), data.data(), data.length());
}
...
auto Statement::BindList(
    ...
    size_t position) -> CassError
{
    return cass_statement_bind_collection(
