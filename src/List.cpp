...
auto StatementList::AppendCounter(int64_t value) -> bool
{
    return cass_collection_append_int64(m_cass_collection_ptr.get(), value) == CASS_OK;
}
...
auto StatementList::AppendVarChar(std::string_view data) -> bool
{
    return AppendASCII(data);
}

auto StatementList::AppendTimeUUID(std::string_view timeuuid) -> bool
{
    return AppendUUID(timeuuid);
}

auto StatementList::AppendINet(std::string_view inet) -> bool
{
    CassInet  cass_inet;
...
ResultList::ResultList(const CassValue* cass_value) : m_cass_value(cass_value)
{
}
